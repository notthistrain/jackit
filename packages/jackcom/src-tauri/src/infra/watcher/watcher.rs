...
impl PortWatcher {
    ...
    pub fn scan_once(&mut self) -> PortChange {
        let current: HashSet<String> = match serialport::available_ports() {
        ...
        let known: HashSet<String> = self.known_ports.iter().cloned().collect();

        let arrived: Vec<PortName> = current
            .difference(&known)
            .map(|p| PortName::new(p.clone()))
            .collect();

        let removed: Vec<PortName> = known
            .difference(&current)
            .map(|p| PortName::new(p.clone()))
            .collect();

        self.known_ports = current.into_iter().collect();
...
mod tests {
    ...
    fn watcher_detects_new_port() {
        ...
        // 首次初始化
        watcher.initialize();
        let _known_count = watcher.known_ports.len();
